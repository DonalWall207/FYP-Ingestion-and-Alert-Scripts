...
def send_email(host, status, malware_family, event_time):
    ...
    subject = f"Splunk Alert: Specific Threat IP {host} Detected"
    body = f"""
    ...
    try:
        ...
        # Ensure server is properly closed after the operation if it was initialized
        if 'server' in locals():
            server.quit()

# Function to check the saved search results
def check_saved_search():
    # Splunk REST API settings
    splunk_url = 'https://127.0.0.1:8089'
    saved_search_name = 'Threat Alert Email Action3' 
    ...
    if result_response.status_code == 200:
        ...
        if 'results' in results and results['results']:
            for result in results['results']:
                ...
                send_email(host, status, malware_family, event_time)
        else:
